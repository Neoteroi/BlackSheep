...
This folder contains scripts to benchmark the performance of the library. The purpose of
these benchmarks is to measure how changes in code affect performance, across Git commits.

Benchmarks measure execution time and memory utilization.

The code can both collect information and compare it depending on the Git commit SHA.
...
PYTHONPATH="." python perf/main.py

# Compare results
PYTHONPATH="." python perf/compare.py
```
...
python -m cProfile -s cumulative perf/benchmarks/writeresponse.py | head -n 20
```
...
```bash
export PYTHONPATH="."
rm -rf benchmark_results && python perf/main.py && python perf/compare.py
```
