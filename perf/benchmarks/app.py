...
REQUEST_HEADERS = [
    ...
    (b"sec-ch-ua", b'"Chromium";v="112", "Google Chrome";v="112", "Not:A-Brand";v="99"'),
    (b"sec-ch-ua-mobile", b"?0"),
    (b"sec-ch-ua-platform", b'"Windows"'),
    (b"Upgrade-Insecure-Requests", b"1"),
    (b"User-Agent", b"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Safari/537.36"),
...
async def benchmark_app_handle_small_response(iterations=1000):
...
async def test_app_handle_text_response(application: Application):
    scope = get_example_scope(
        "GET", "/test", extra_headers=REQUEST_HEADERS
    )
    mock_send = MockSend()
    await application(scope, MockReceive(), mock_send)
...
async def benchmark_app_handle_text_response(iterations=1000):
    application = Application(router=Router())
...
if __name__ == "__main__":
    main_run(benchmark_app_handle_small_response)
